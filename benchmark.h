...
Measurement<long> measure(const char* name, const std::function<long(int)>& fn, int input, int test, long result, const int samples = 10000)
{
    assert(samples >= 1000);
    if (fn(test) != result)
    {
        std::cout << name << " failed:\n INPUT  [" << test << "]\n OUTPUT [" << fn(test) << "]\n EXPECT [" << result << "]" << std::endl;
        return Measurement<long>{name, result, 0, 0, 0, 0, 0, 0 };
    }
    ...
    return Measurement<long>{name, result, midtime / (double)samples, entries[samples / 2], mintime, q1, maxtime, q3};
}

#define MEASURE(func, test, result, input) measure(#func, func, input, test, result)
